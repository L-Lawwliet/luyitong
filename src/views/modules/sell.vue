      ...
      <scroll :on-refresh="onRefresh" :on-infinite="onInfinite" :loadText="loadText" tipColor="#fff">
        <el-row class="sell-item-wrapper" v-for="item in goods" :key="item.id">
          <el-row>
            ...
            <el-col :span="9" class="time ta-r">{{fmtDate(item.RELEASE_TIME)}}</el-col>
          </el-row>
          <el-row class="order">订单ID：{{item.PRODUCT_ID}}</el-row>
          <el-row>
            <el-col :span="12">
        ...
        <div class="dialog-content ta-c">
          ...
          <div class="order-goods" v-show="orderGoods.length>0">
            <div v-for="item in orderGoods" :key="item.id">
              <el-col :span="12" class="product-name">{{item.PRODUCT_NAME}}</el-col>
