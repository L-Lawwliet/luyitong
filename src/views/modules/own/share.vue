    ...
    <div class="share-wrapper">
      <scroll :on-refresh="onRefresh">
        ...
        <group class="friends">
          ...
          <div v-for="(friend, index) in friendRecord" :key="friend.userName">
            <cell :title="friend.userName" is-link :border-intent="false" :arrow-direction="friend.expand ? 'up' : 'down'" @click.native="friendClick(index)">
              <i slot="icon" class="iconfont icon-touxiang-"></i>
            </cell>
            <div class="slide" :class="friend.expand?'animate':''">
              <el-row class="share-friends-item" v-for="item in friend.data" :key="item.updatedatetime">
                <el-col :span="12">{{fmtDate(item.updatedatetime)}}</el-col>
                <el-col :span="12" class="ta-r">{{item.changeWorth}}元</el-col>
              </el-row>
            </div>
          </div>
        </group>
...
import { Group, Cell, XHeader, XButton, Alert, Confirm } from "vux";
  ...
  methods: {
    ...
    getAssets() {
      api.getAssets().then(res => {
        if (res.data.msg_code === 'success') {
          this.profit = res.data.result.CASH_VALUE;
...
.share-friends-item:first-of-type {
  padding-bottom: 0;
}
</style>
